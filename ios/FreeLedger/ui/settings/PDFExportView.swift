...
struct PDFExportView: View {
    ...
    private var pdfFilename: String {
        ...
        case .currentMonth:
            ...
            return "FreeLedger-\(y)\(String(format: "%02d", m))-\(dateStr).pdf"
        case .specificMonth:
            return "FreeLedger-\(selectedYear)\(String(format: "%02d", selectedMonth))-\(dateStr).pdf"
        case .specificYear:
            return "FreeLedger-\(selectedYear)-\(dateStr).pdf"
        case .all:
            return "FreeLedger-All-\(dateStr).pdf"
        }
    }
